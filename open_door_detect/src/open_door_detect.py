...
def Is_door_check():
    ...
    for i in range(225, 359):
        ...
        if lidar_ranges[i + 1] > lidar_ranges[i] + 1:    # 1m = 1
            ...
            # 두번째 문틀탐색 => i 번째부터 시작
            for j in range(i+2, 360):
                # 0 ~ 180범위에서도 문 검출 안될 시 해당 for문 빠져나가기 위해
                if break_flag == True:
                    break
                else:
                    # 225 ~ 360범위안에 두번째 문틀 검출
                    if lidar_ranges[j] < lidar_ranges[i + 1] and lidar_ranges[j] < lidar_ranges[i]:
                        print("#")
                        print(i)
                        print(j)
                        print("#")
                        ...
                        if door_width > 0.5 and door_width < 3:
                            isdoor = True
